...
compatibility: Requires `uv`; commands install Pillow ephemerally with `uv run --with pillow`.
---
...
uv run --with pillow python "$PHOTO_SCRIPT" <command> ...
```

Inspect before choosing geometry or an output format:

```sh
uv run --with pillow python "$PHOTO_SCRIPT" inspect path/to/image.png
```
...
uv run --with pillow python "$PHOTO_SCRIPT" \
